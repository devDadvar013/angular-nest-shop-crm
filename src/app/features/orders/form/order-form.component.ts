  ...
  template: `
    <div class="mx-auto max-w-3xl space-y-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-ink-900">سفارش جدید</h1>
          <p class="text-sm text-ink-500">مشتری و اقلام سفارش را انتخاب کنید</p>
        </div>
      </div>
      ...
      <div class="card p-6">
        ...
        <div class="flex flex-wrap items-end gap-3 border-b border-ink-100 pb-4">
          ...
          <button type="button" class="btn-secondary" (click)="addLine()">افزودن</button>
        </div>

        @if (lines().length === 0) {
          <p class="py-8 text-center text-sm text-ink-400">هنوز آیتمی اضافه نشده است.</p>
        } @else {
          ...
          <div class="mt-4 flex justify-end border-t border-ink-100 pt-4">
            <div class="text-left">
              ...
              <p class="text-lg font-bold text-ink-900">{{ total() | toman }}</p>
            </div>
          </div>
        }
      </div>
